...
export default function ManagerEventsTable({ 
  events, 
  showForm, 
  form, 
  setForm, 
          ...
          <form onSubmit={handleCreateEvent} className="admin-form">
            ...
            <div className="form-group">
              ...
              <select className="form-select" value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                {["hackathon", "course", "campaign", "training", "challenge"].map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div className="form-group">
              <label className="form-label">Department</label>
              <select className="form-select" value={form.department} onChange={(e) => setForm({ ...form, department: e.target.value })}>
                {["All", "Engineering", "Design", "Marketing", "HR", "Finance", "Operations"].map((d) => <option key={d}>{d}</option>)}
              </select>
            </div>
            <div className="form-group">
            ...
            <div className="form-group">
              ...
              <input type="number" className="form-input" min="1" value={form.reward_points} onChange={(e) => setForm({ ...form, reward_points: e.target.value })} required />
            </div>
            <div className="form-group">
              <label className="form-label">Max Participants</label>
              <input type="number" className="form-input" placeholder="Unlimited" value={form.max_participants} onChange={(e) => setForm({ ...form, max_participants: e.target.value })} />
            </div>
            <div className="form-group" style={{ gridColumn: "1/-1" }}>
              <label className="form-label">Tags (comma-separated)</label>
              <input className="form-input" placeholder="python, learning, backend" value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} />
            </div>
            ...
            <tbody>
              {events.map((i) => (
                <tr key={i.id}>
                  ...
                  <td><span className={`badge ${i.difficulty === "Easy" ? "badge-success" : i.difficulty === "Medium" ? "badge-warning" : "badge-danger"}`}>{i.difficulty}</span></td>
